...
class RNNReadModule(torch.nn.Module):
    def __init__(self, embedding_mat, vector_input_shape, otherfeature_shape):
        ...
        inner_shape = 400
        self.rnn = torch.nn.LSTM(300, inner_shape, 1, dropout=0.25)

        self.h_transform = torch.nn.Linear(otherfeature_shape, inner_shape*1)
        self.c_transform = torch.nn.Linear(otherfeature_shape, inner_shape*1)
        ...
        self.output_transforms = [
            torch.nn.Linear(inner_shape+32, 128),
            torch.nn.PReLU(),
            torch.nn.Dropout(0.25),
    ...
    def forward(self, x1, x2, features):
        ...
        h = self.h_transform(torch.clamp(features, -5, 5))
        c = self.c_transform(torch.clamp(features, -5, 5))
